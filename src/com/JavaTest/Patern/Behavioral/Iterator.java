...
interface ProfileIterator {
 ...
 class FacebookIterator implements ProfileIterator {
    ...
    private void lazyLoad() {
        if (emails.size() == 0) {
            List<String> profiles = facebook.requestFriendsEmailsFromFacebook(this.email, this.type);
...
class LinkedInIterator implements ProfileIterator {
    ...
    public LinkedInIterator(LinkedIn linkedIn, String type, String number) {
    ...
    private void lazyLoad() {
        if (numbers.size() == 0) {
            List<String> profiles = linkedIn.requestRelatedContactsFromLinkedInAPI(this.number, this.type);
            for (String profile : profiles) {
                this.numbers.add(profile);
    ...
    public Profile getNext() {
        ...
        String friendEmail = numbers.get(currentPosition);
...
class Facebook implements SocialNetwork {
    ...
    public List<String> requestFriendsEmailsFromFacebook(String profileEmail, String contactType) {
        // Здесь бы был POST запрос к одному из адресов API Facebook. Но вместо
        // этого мы эмулируем долгое сетевое соединение, прямо как в реальной
        // жизни...
        simulateNetworkLatency();
